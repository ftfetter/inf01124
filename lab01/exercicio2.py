...
def shellSort(originalArray, size, sequence):
...
inputFile = 'entrada2.txt'
outputFile = 'saida2.txt'
...
with open(inputFile, 'r') as input:
    ...
    for line in lines:
        ...
        start = time.time()
        shellSort(array, size, SHELL)
        elapsedTime = time.time() - start
        output.write('SHELL,' + str(size) + ',' + str(elapsedTime) + '\n')

        start = time.time()
        shellSort(array, size, KNUTH)
        elapsedTime = time.time() - start
        output.write('KNUTH,' + str(size) + ',' + str(elapsedTime) + '\n')

        start = time.time()
        shellSort(array, size, CIURA)
        elapsedTime = time.time() - start
        output.write('CIURA,' + str(size) + ',' + str(elapsedTime) + '\n')
